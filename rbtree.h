...
/**
 * delete have not been implemented.
 **/
class bst
{
private:
    ...
    uint find(Node *p, double w)
    {
        ...
        if (w > p->value)
            if (p->rightTree == NIL)
            {
                ...
                return find(p->rightTree, w);

        if (w < p->value)
            if (p->leftTree == NIL)
                return p->idx;
            else
                return find(p->leftTree, w);
    }
};